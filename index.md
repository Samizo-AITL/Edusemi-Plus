---
layout: default
title: Edusemi-Plus｜半導体産業と社会・AIをつなぐ応用教材シリーズ
---

{% assign ROOT = '' | absolute_url %}

# 📚 **Edusemi-Plus｜半導体産業と社会・AIをつなぐ応用教材シリーズ**  
*Edusemi-Plus: Applied learning series connecting semiconductors with society & AI*

**JP:** **Edusemi-Plus** は、基礎教材 **Edusemi-v4x** を拡張し、**地政学・企業戦略・製品構造・AI・量子・材料・投資** などの視点から、現代テクノロジー産業を多角的に探究する **応用教材群** です。  
**EN:** **Edusemi-Plus** extends **Edusemi-v4x** to explore the tech industry from **geopolitics, corporate strategy, product architecture, AI, quantum, materials, and investment** perspectives.

- 🌍 **JP:** **技術と社会構造** の接点に注目｜**EN:** Focus on the nexus of **technology & societal structures**  
- 🧠 **JP:** **ChatGPT × Gemini × Claude** による探究型学習｜**EN:** Inquiry learning with **ChatGPT × Gemini × Claude**  
- 🧑‍🏫 **JP:** **教育・戦略・技術設計** に活用可能｜**EN:** Usable for **education, strategy, and engineering**

🇺🇸 **EN page:** [**English version**]({{ ROOT }}/index_en.html)

---

## 🎯 **本教材の目的 / Objectives**

|  | **JP（日本語）** | **EN (English)** |
|---|---|---|
| 🧭 **背景理解 / Context** | 技術の背後にある **地政学・経済・政策** を読み解く | Read technology through **geopolitics, economy, policy** |
| 🧠 **思考力 / Reasoning** | 技術選定・製品設計・戦略判断の「**なぜ**」を深掘り | Dive into the "**why**" of tech, design, and strategy |
| 🔄 **基礎接続 / Bridging** | **Edusemi（基礎）** と **産業構造** を橋渡し | Bridge **Edusemi (fundamentals)** with **industry structure** |

> 🆕 **JP:** **Rapidusによる2nm試作チップ動作確認（2025年7月）** を教材化。詳しくは **[Rapidus]({{ ROOT }}/rapidus/index.html)**。  
> 🆕 **EN:** **Rapidus: 2nm prototype chip functional confirmation (Jul 2025)** covered as a case study. See **[Rapidus]({{ ROOT }}/rapidus/index.html)**.

---

## 📚 **コンテンツカテゴリ / Content Categories**

| **カテゴリ / Category** | **概要（JP）** | **Summary (EN)** | **Link** |
|---|---|---|---|
| 🧪 **材料技術 / Materials** | **Si / SiC / GaN** の比較・応用 | Compare & apply **Si / SiC / GaN** | [materials]({{ ROOT }}/materials/index.html) |
| ⚛️ **先端技術 / Advanced** | **量子・Cryo・次世代構造** | **Quantum, cryo, next-gen architectures** | [quantum-semiconductor]({{ ROOT }}/quantum-semiconductor/index.html) |
| 🤖 **AI / AI** | **LLM活用・AIアクセラレータ** | **LLMs & AI accelerators** | [ai-semiconductor]({{ ROOT }}/ai-semiconductor/index.html), [ai-tools-comparison]({{ ROOT }}/ai-tools-comparison/index.html) |
| 🏭 **産業 / Industry** | **TSMC / Apple の構造・復興** | Structures & resurgence of **TSMC/Apple** | [tsmc-insight]({{ ROOT }}/tsmc-insight/index.html), [rapidus]({{ ROOT }}/rapidus/index.html) |
| 🌐 **地政学 / Geopolitics** | **輸出管理・国家間競争** | **Export controls & competition** | [geopolitics]({{ ROOT }}/geopolitics/index.html) |
| 💹 **投資 / Investment** | **企業評価・設備計画** | **Equity & capex planning** | [investment]({{ ROOT }}/investment/index.html) |
| 🗂️ **アーカイブ / Archive** | **立ち上げ技術・産業史** | **Ramps & history** | [archive]({{ ROOT }}/archive/index.html) |

---

## 🔗 **関連構造カテゴリ / Industry Structure Map**

📌 **JP:** 「産業構造マップ（industry/）」は EDA・装置・材料・解析・デバイスまでを体系化。  
📌 **EN:** The **Industry Structure Map** systematizes EDA, equipment, materials, metrology, and devices.

| **カテゴリ / Category** | **JP（概要）** | **EN (Summary)** | **Link** |
|---|---|---|---|
| 🏭 **全体概要 / Overview** | 設計〜製造〜検査の俯瞰 | End-to-end overview | [industry]({{ ROOT }}/industry/index.html) |
| 🧠 **EDAツール / EDA Tools** | 自動化設計・検証・P&R・TCAD | EDA vendors & flows | [industry/eda-tools]({{ ROOT }}/industry/eda-tools/index.html) |
| 🖼️ **フォトマスク / Photomasks** | ブランクス・OPC・EUV描画 | Blanks, OPC, EUV write | [industry/photomasks]({{ ROOT }}/industry/photomasks/index.html) |
| ⚙️ **前工程装置 / Front Equip.** | 成膜・露光・洗浄・エッチング | Depo, litho, clean, etch | [industry/front-equipments]({{ ROOT }}/industry/front-equipments/index.html) |
| 🧪 **前工程材料 / Front Materials** | Siウェーハ・ガス・レジスト | Wafers, gases, resists | [industry/front-materials]({{ ROOT }}/industry/front-materials/index.html) |
| 🏗️ **後工程装置 / Back Equip.** | ダイシング・ボンディング等 | Dicing, bonding, pkging | [industry/back-equipments]({{ ROOT }}/industry/back-equipments/index.html) |
| 🧯 **後工程材料 / Back Materials** | 基板・封止樹脂・配線材 | Substrates, mold, wire | [industry/back-materials]({{ ROOT }}/industry/back-materials/index.html) |
| 🔬 **解析計測 / Metrology** | CD-SEM・欠陥検査・膜厚 | CD-SEM, defect, thickness | [industry/metrology-tools]({{ ROOT }}/industry/metrology-tools/index.html) |
| 💾 **デバイス / Devices** | ファウンドリ・IDM・ファブレス | Foundry, IDM, fabless | [industry/device-makers]({{ ROOT }}/industry/device-makers/index.html) |

---

## 👥 **対象読者 / Audience & Use Cases**

| **対象 / Audience** | **JP（ニーズ）** | **EN (Needs)** |
|---|---|---|
| 🧑‍🔬 **技術者 / Engineers** | **戦略・社会的構造** の理解（例：**Rapidus**） | Understand **strategic & social structures** (e.g., **Rapidus**) |
| 🧑‍💼 **企画 / Management** | **技術選定・産業調査・海外戦略** | **Tech selection, industry research, global strategy** |
| 🧑‍🏫 **教育 / Educators & Students** | **産業と社会構造** を学ぶ教材 | Materials on **industry & society** |

---

## 🧠 **AIツール連携 / Working with AI Tools**

| **役割 / Role** | **ツール / Tools** | **活用ポイント / Usage** |
|---|---|---|
| 🧩 **メイン生成 / Authoring** | **ChatGPT 4o** | 構成・比較表・演習・図解 / Structure, tables, exercises, visuals |
| 🔍 **情報検索 / Search** | **Gemini 1.5 Pro** | 地政学・材料・海外技術 / Geopolitics, materials, global tech |
| ✒️ **整形 / Polishing** | **Claude 3 Opus** | 長文整理・報告構成 / Long-form editing, reports |

> 🔗 **Use cases / 使い分け**： [ai-tools-comparison]({{ ROOT }}/ai-tools-comparison/index.html)

---

## 🧩 **活用方法 / How to Use**

- **JP:** 教育・研修：テーマ別スライド／ノート教材｜技術戦略：会議・分析の土台｜探究学習：**対話型・構造化学習**  
- **EN:** Education & training (slides/notes), strategy support (meetings/analysis), inquiry learning with **conversational & structured** flows

---

## 📄 **ライセンス / License**

- **JP:** 本教材は [**MIT License**](https://opensource.org/licenses/MIT) に基づき、**教育・非営利での再利用・編集・派生**を歓迎します。  
- **EN:** Licensed under **[MIT License](https://opensource.org/licenses/MIT)**. Free to **reuse, modify, and fork** for education/non-profit.

> **JP:** 記述は執筆者の調査・思考に基づきます。｜**EN:** Opinions are those of the author.

---

## 🔗 **関連教材・プロジェクト / Related Projects**

| **Project** | **JP（概要）** | **EN (Summary)** |
|---|---|---|
| **Edusemi-v4x** | **半導体基礎技術教材**：設計・物性・PDK・テスト | **Foundational semiconductors**: design, physics, PDK, test |
| **EduController** | **制御理論 × AI制御**（全9章） | **Control theory × AI control** (9 chapters) |
| **AITL-H** | **知能制御：FSM＋PID＋LLM** | **Intelligent control: FSM + PID + LLM** |

**Links:**  
- [Edusemi-v4x (GitHub)](https://github.com/Samizo-AITL/Edusemi-v4x)  
- [EduController (GitHub)](https://github.com/Samizo-AITL/EduController)  
- [AITL-H (GitHub)](https://github.com/Samizo-AITL/AITL-H)

---

## 👤 **執筆者 / Author**

**三溝 真一（Shinichi Samizo） / Shinichi Samizo**  
- **JP:** 信州大学大学院 電気電子工学 修了｜元 **セイコーエプソン** 技術者（1997年〜）  
- **EN:** M.Eng., Electrical & Electronic Eng., Shinshu Univ.｜Former engineer at **Seiko Epson**

**経験 / Experience:**  
- **JP:** 半導体（ロジック・メモリ・高耐圧混載）／薄膜ピエゾ／製品化・BOM・ISO  
- **EN:** Semiconductors (logic/memory/HV-AMS), thin-film piezo, productization/BOM/ISO

**連絡先 / Contact:**  
- ✉️ [shin3t72@gmail.com](mailto:shin3t72@gmail.com)  
- 🐦 https://x.com/shin3t72  
- 💻 https://samizo-aitl.github.io/

---

> **JP:** 「**技術** と **世界の構造** をつなげて理解し、未来戦略を描く。」  
> **EN:** “Connect **technology** with **global structures** to shape future strategies.”
